...
because declarative languages typically don’t have for-loops, how do you repeat a piece of logic—such as creating multiple similar resources—without copy and paste?

Terraform comes with several different looping constructs, each used slightly different.
...
One problem is that all three IAM users would have the same name, which would cause an error, since usernames must be unique. If you had access to a standard for-loop, you might use the index in the for-loop, i, to give each user a unique name:
...
the count parameter can also be used on modules. Just as adding count to a resource turns it into an array of resources, adding count to a module turns it into an array of modules. 

+ for_each - to loop over resources, inline blocks within a resource, and modules
...
can be more complex with tagging. does not work for count, but for for each. not mentioned as general intro not focus on.

+ for - to loop over lists and maps

Terraform offers similar functionality as python list comprehension in the form of a for expression (not to be confused with the for_each expression you saw in the previous section). The basic syntax of a for expression is as follows:
