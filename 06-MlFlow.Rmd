...
mlflow run .
mlflow run . --experiment-name="models-experiment"
MLFLOW_TRACKING_URI=http://localhost:5005 mlflow run . --experiment-name="models-experiment"
mlflow run . --env-manager virtualenv


# Run the MLflow project from a Github URI
MLflow run git@github.com/seblum/mlops-practice//files/06-MLFlow/mlprojects.git?ref=chapter/mlflow

MLFLOW_TRACKING_URI=http://localhost:5000 MLflow run https://github.com/seblum/mlops-practice#files/06-MLFlow/mlprojects --version=chapter/mlflow
```
...
files/06-MLFlow/mlprojects
TODO: When running a server you have to provide the URI to the server as environment variable to mflow run.

TODO: by setting the MLFLOW\_TRACKING\_URI environment variable we specify an execution backend for the run

TODO: * **experiment_id** – ID of experiment under which to launch the run.
